...
var testCase = {
  ...
  testPath: function() {
    ...
    assertEqual('f', Toy.path('fn').next(call(f, x)).toString());
    assertEqual('x', Toy.path('arg').next(call(f, x)).toString());
    var p2 = '/left/binop';
  ...
  testDeepTermReplacer: function() {
    // Leverages the code for testConjunctsReplacer.
    // 
...
Toy.eachFact(function(fact) {
    var str = Toy.getStatement(fact).toString();
    console.log('Testing ' + str);
      test(str, function() {
          assert(fact.result().ruleName);
        });
  });
