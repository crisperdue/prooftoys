...
export interface Expr {
  /**
   * Returns a shallow copy of this, annotating the copy with the given
   * rule name, rule arguments, and dependencies (ruleName, ruleArgs,
   * ruleDeps).  "This" becomes the "details" property if it already has
   * a justification.  (Axioms, definitions and assertions are not
   * generated by rules, though theorems are.)  The ruleDeps array is
   * optional, empty if not given.
   *
   * If this is identical to one of its ruleDeps, returns this
   * unchanged, making the step a no-op, unless "retain" is true.  (The
   * purpose is to reduce clutter for readers of the proof.)  Some
   * steps, especially simplifications, do this same thing internally by
   * skipping the call to "justify".
   *
   * TODO: Make a Step class for proof steps, and have this method
   * return a new one.
   *
   * TODO: Consider always creating a new step, but omitting it in some
   * situations when rendering.
   */
